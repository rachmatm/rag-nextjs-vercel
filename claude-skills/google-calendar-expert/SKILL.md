...
1. Compute the file path: `./.claude/google-calendar-expert-local-knowledge/google-calendar-expert.json` (relative to the workspace root).
2. If the directory `./.claude/google-calendar-expert-local-knowledge/` does not exist, create it.
...
6. Tell the user the entry was captured to `./.claude/google-calendar-expert-local-knowledge/google-calendar-expert.json` and remind them that the canonical source of truth is still `knowledge/google-calendar.json`, which they own and must update before running `npm run export:neon`.
