...
before answering Next.js/Vercel questions or attempting a fix.
...
- **Active_Stack:** `nextjs-vercel`
- ALWAYS pass `stack: "nextjs-vercel"` to `search_knowledge_base` and `list_knowledge_filters`.
- **Forbidden_Stack:** `react-native` — NEVER pass `stack: "react-native"` on any tool call. That is a different expert's domain.
...
1. Compute the file path: `./.claude/nextjs-expert-local-knowledge/nextjs-expert.json` (relative to the workspace root).
2. If the directory `./.claude/nextjs-expert-local-knowledge/` does not exist, create it.
3. If the JSON file does not exist, initialise it with an empty JSON array: `[]`.
4. Build a knowledge entry matching the canonical schema used in `knowledge/nextjs-vercel.json`:
   - `type`, `symptoms[]`, `root_cause`, `fix[]`, `tags[]`, `severity` (one of `low` | `medium` | `high`), `frequency` (one of `rare` | `occasional` | `common` | `very-common`), `related_docs[]`, `version`, and `stack: "nextjs-vercel"`.
5. Append the new entry to the array. The result must remain valid JSON (balanced brackets, no trailing commas).
6. Tell the user the entry was captured to `./.claude/nextjs-expert-local-knowledge/nextjs-expert.json` and remind them that the canonical source of truth is still `knowledge/nextjs-vercel.json`, which they own and must update before running `npm run export:neon`.
...
- Never write to `knowledge/nextjs-vercel.json` directly — that file is owned by the user.
