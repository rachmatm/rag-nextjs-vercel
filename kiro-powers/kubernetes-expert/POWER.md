...
A senior **on-premise Kubernetes** engineering persona, backed by a live,
curated RAG knowledge base served over the `dev-knowledge` MCP server.

## What it does

- Answers Kubernetes questions grounded in vetted `root_cause` + `fix` entries.
- Queries the knowledge base **before** attempting a fix or recommending a pattern.
- Stays strictly within the `kubernetes` stack — it never mixes in knowledge from other stacks.
...
| Tool | Purpose |
|------|---------|
| `search_knowledge_base` | Full-text search; `query` + `stack` required. Use `stack: "kubernetes"`. |
...
1. Activate the power, then ask any Kubernetes question.
...
3. The steering instructs the agent to call `search_knowledge_base` with
   `stack: "kubernetes"` first, expand results with `get_knowledge_entry`, and
...
ever queries `kubernetes`. For Next.js/Vercel work use the **Next.js / Vercel Expert**
power; for Google API work use the corresponding **Google OAuth/Calendar/Sheets Expert** powers.
