...
primary reference before answering Google Calendar questions or attempting a fix.
...
1. Compute the file path: `./.kiro/google-calendar-expert-local-knowledge/google-calendar-expert.json` (relative to the workspace root).
2. If the directory `./.kiro/google-calendar-expert-local-knowledge/` does not exist, create it.
3. If the JSON file does not exist, initialise it with an empty JSON array: `[]`.
4. Build a knowledge entry matching the canonical schema used in `knowledge/google-calendar.json`:
   - `type`, `symptoms[]`, `root_cause`, `fix[]`, `tags[]`, `severity` (one of `low` | `medium` | `high`), `frequency` (one of `rare` | `occasional` | `common` | `very-common`), `related_docs[]`, `version`, and `stack: "google-calendar"`.
5. Append the new entry to the array. The result must remain valid JSON (balanced brackets, no trailing commas).
6. Tell the user the entry was captured to `./.kiro/google-calendar-expert-local-knowledge/google-calendar-expert.json` and remind them that the canonical source of truth is still `knowledge/google-calendar.json`, which they own and must update before running `npm run export:neon`.
...
- Never write to `knowledge/google-calendar.json` directly — that file is owned by the user.
