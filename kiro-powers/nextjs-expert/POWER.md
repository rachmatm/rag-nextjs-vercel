...
A senior **Next.js (App Router) + Vercel** engineering persona, backed by a live,
curated RAG knowledge base served over the `dev-knowledge` MCP server.

## What it does

- Answers Next.js / Vercel questions grounded in vetted `root_cause` + `fix` entries.
- Queries the knowledge base **before** attempting a fix or recommending a pattern.
- Stays strictly within the `nextjs-vercel` stack — it never mixes in React Native knowledge.
...
| Tool | Purpose |
|------|---------|
| `search_knowledge_base` | Full-text search; `query` + `stack` required. Use `stack: "nextjs-vercel"`. |
...
1. Activate the power, then ask any Next.js / Vercel question.
...
3. The steering instructs the agent to call `search_knowledge_base` with
   `stack: "nextjs-vercel"` first, expand results with `get_knowledge_entry`, and
...
ever queries `nextjs-vercel`. For React Native work, use the **React Native Expert**
power instead.
