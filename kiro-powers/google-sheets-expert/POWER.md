...
A senior **Google Sheets API** engineering persona, backed by a live,
curated RAG knowledge base served over the `dev-knowledge` MCP server.

## What it does

- Answers Google Sheets API questions grounded in vetted `root_cause` + `fix` entries.
- Queries the knowledge base **before** attempting a fix or recommending a pattern.
- Stays strictly within the `google-sheets` stack — it never mixes in `nextjs-vercel`, `react-native`, `google-oauth`, or `google-calendar` knowledge.
...
| Tool | Purpose |
|------|---------|
| `search_knowledge_base` | Full-text search; `query` + `stack` required. Use `stack: "google-sheets"`. |
...
1. Activate the power, then ask any Google Sheets API question.
...
3. The steering instructs the agent to call `search_knowledge_base` with
   `stack: "google-sheets"` first, expand results with `get_knowledge_entry`, and
...
ever queries `google-sheets`. For OAuth work, use the **Google OAuth Expert** power;
for Calendar API work, use the **Google Calendar Expert** power.
